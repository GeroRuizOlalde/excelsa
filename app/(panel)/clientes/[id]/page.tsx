...
export default function DetalleClientePage() {
  ...
  // Estados para Modal de Anulación (Mejora UX)
    ...
    const fetchData = async () => {
      try {
        ...
        const [resCliente, resFacturas, resConfig] = await Promise.all([
          ...
          supabase.from('configuracion').select('color_primario').single() // Traemos color
        ]);
  ...
  // 1. Paso 1: Abrir el modal en lugar del prompt
  ...
  return (
    // INYECTAMOS LA VARIABLE CSS --primary AQUÍ
    <div style={{ '--primary': accentColor } as React.CSSProperties} className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 md:p-8 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto">
           <button className="flex-1 md:flex-none justify-center flex bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3 rounded-xl text-slate-500 hover:text-primary transition-colors">
             <Edit2 size={20} />
           </button>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-slate-900 rounded-[2rem] p-8 border border-white/5 relative overflow-hidden shadow-2xl">
            {/* Efecto de luz con el color primario */}
      ...
      {/* --- MODAL DE ANULACIÓN (UX MEJORADA) --- */}
      {facturaAAnular && (
