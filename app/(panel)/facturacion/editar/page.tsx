...
// ------------------------------------------------------------------
// 1. COMPONENTE INTERNO (CONTIENE TODA LA LÓGICA)
// ------------------------------------------------------------------
function ContenidoEditarFactura() {
  const router = useRouter();
  const searchParams = useSearchParams(); // <--- NUEVO: Hook para leer ?id=123
  ...
  useEffect(() => {
    const idParam = searchParams.get('id'); // Leemos ?id=...
    if (idParam) {
      setFacturaId(idParam);
    } else {
      toast.error("No se especificó una factura para editar");
      router.push('/facturacion');
    }
  }, [searchParams, router]);

  // 2. CARGAR DATOS (Igual que antes, pero depende de facturaId)
  useEffect(() => {
    if (!facturaId) return;
  ...
  // 3. LÓGICA DE GUARDADO (Igual que antes)
  const actualizarFactura = async () => {
    if (!clienteId || items.length === 0) return toast.warning("Faltan datos obligatorios");
    setGuardando(true);
  ...
  if (loading) return (
    <div className="h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-400 gap-3">
      <Loader2 className="animate-spin text-primary" size={32} />
      <span className="font-bold text-xs uppercase tracking-widest">Cargando Factura...</span>
    </div>
  );
...
export default function EditarFacturaPage() {
  return (
    <Suspense fallback={
      <div className="h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-400 gap-3">
        <Loader2 className="animate-spin text-primary" size={32} />
        <span className="font-bold text-xs uppercase tracking-widest">Cargando editor...</span>
      </div>
    }>
      <ContenidoEditarFactura />
    </Suspense>
  );
}